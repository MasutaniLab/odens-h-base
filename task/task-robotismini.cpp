///
///@file task-robotismini.cpp
///@brief ROBOTIS MINI用 Taskクラスのメンバ関数の定義（公開用）
///@par Copyright
/// Copyright (C) 2016, 2017 Team ODENS, Masutani Lab, Osaka Electro-Communication University
///@par 履歴
///- 2017/03/05 升谷 保博
///@addtogroup robotismini
///@{
///
...
namespace odens {
  ...
  ///@retval true 倒れている
  ///
  bool TaskROBOTISMINI::isLying(const srInfo &info) {
    return false; //ROBOTIS MINIでは転倒の判断必要なし
  }
  ...
  ///
  ///@brief 起き上がり（ROBOTIS MINIは自律で起き上がるので何もしない）
  ///@param[in] info フィールドの位置情報
  ///@param[in] ctime 現在の時刻
  ///@return 終了状態
  ///
  int TaskROBOTISMINI::standUp(const srInfo &info, double ctime)
  {
    return 0;
  }
  ...
  ///@return 終了状態
  ///
  int TaskROBOTISMINI::move(const srInfo &info, const Orthogonal &target)
  {
    Command com;
